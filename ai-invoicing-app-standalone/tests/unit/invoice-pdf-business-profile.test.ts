...
describe('invoice PDF business identity', () => {
  it('generates a valid PDF when a complete Aleya business profile is supplied', async () => {
    ...
    const text = extractPdfText(withProfile);
    expect(text).toContain('Quantum Hire Services');
    expect(text).toContain('1 Scaffold Way Sydney');
    expect(text).toContain('12345678901');
    expect(text).toContain('accounts@quantum.example');
    expect(text).toContain('0400000000');
    expect(text).toContain('TAX INVOICE');
    expect(text).toContain('Payment details');
    expect(text).toContain('Payment due within 14 days');
