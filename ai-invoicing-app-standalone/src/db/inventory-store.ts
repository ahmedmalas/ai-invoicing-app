...
import { evaluateStockIntelligence, profitMarginPercent } from '../domain/inventory/stock-intelligence.js';
import type {
  InventoryAlert,
...
export function ensureInventorySchemaSqlite(db: SqliteDb): void {
  ...
  db.exec(`
    INSERT OR IGNORE INTO goods_receipt_sequences (id, prefix, year, next_sequence)
    VALUES (1, 'GRN', CAST(strftime('%Y', 'now') AS INTEGER), 1);
    INSERT OR IGNORE INTO stocktake_sequences (id, prefix, year, next_sequence)
    VALUES (1, 'STK', CAST(strftime('%Y', 'now') AS INTEGER), 1);
  `);
...
export function createInventoryStore(
  ...
  return {
    createProduct(input: Record<string, unknown>): Product {
      ...
      const qrPayload =
        (input.qrPayload as string | null | undefined) ??
        buildDefaultQrPayload({ id, sku });
      try {
        db.prepare(
          ...
          id,
          sku,
          (input.barcode as string | null | undefined) ?? null,
          qrPayload,
          String(input.name),
          (input.description as string | null | undefined) ?? null,
          (input.category as string | null | undefined) ?? null,
          (input.brand as string | null | undefined) ?? null,
          (input.supplierId as string | null | undefined) ?? null,
          String(input.unitOfMeasure ?? 'ea'),
          Number(input.costPrice ?? 0),
          Number(input.sellPrice ?? 0),
          String(input.gstStatus ?? 'gst'),
          input.trackStock === false ? 0 : 1,
          Number(input.minimumStockLevel ?? 0),
          Number(input.reorderQuantity ?? 0),
          (input.storageLocation as string | null | undefined) ?? null,
          (input.weight as number | null | undefined) ?? null,
          (input.lengthMm as number | null | undefined) ?? null,
          (input.widthMm as number | null | undefined) ?? null,
          (input.heightMm as number | null | undefined) ?? null,
          (input.imageUrl as string | null | undefined) ?? null,
          (input.notes as string | null | undefined) ?? null,
          input.isActive === false ? 0 : 1,
          input.isBundle === true ? 1 : 0,
          (input.bundleKind as string | null | undefined) ?? null,
          now,
          now,
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (message.includes('products.sku') || message.includes('UNIQUE constraint failed: products.sku')) {
      ...
      const components = (input.bundleComponents as Array<{
    ...
    updateProduct(id: string, input: Record<string, unknown>): Product {
      ...
      const now = nowIso();
      const next = {
        sku: input.sku !== undefined ? String(input.sku) : String(existing.sku),
        barcode:
        ...
        name: input.name !== undefined ? String(input.name) : String(existing.name),
        description:
        ...
        unitOfMeasure:
          input.unitOfMeasure !== undefined
            ? String(input.unitOfMeasure)
            : String(existing.unit_of_measure),
        costPrice:
        ...
        gstStatus:
          input.gstStatus !== undefined
            ? String(input.gstStatus)
            : String(existing.gst_status),
        trackStock:
          input.trackStock !== undefined ? Boolean(input.trackStock) : asBool(existing.track_stock as number),
        minimumStockLevel:
        ...
        isActive:
          input.isActive !== undefined ? Boolean(input.isActive) : asBool(existing.is_active as number),
        isBundle:
          input.isBundle !== undefined ? Boolean(input.isBundle) : asBool(existing.is_bundle as number),
        bundleKind:
    ...
    listProducts(filter: {
      ...
      if (filter.q) {
        clauses.push('(p.name LIKE ? OR p.sku LIKE ? OR IFNULL(p.barcode, "") LIKE ? OR IFNULL(p.description, "") LIKE ?)');
        const like = `%${filter.q}%`;
    ...
    setJobMaterials(
      ...
      for (const material of materials) {
        const id = randomUUID();
        insert.run(id, jobId, material.productId, material.quantity, material.notes ?? null, now, now);
        result.push({
          id,
          jobId,
    ...
    submitStocktake(id: string): Stocktake {
      ...
      deps.timeline('inventory.stocktake_submitted', id, { stocktakeNumber: stocktake.stocktakeNumber });
      return this.getStocktakeById(id)!;
    },
    ...
    approveStocktake(id: string, approvedBy?: string | null): Stocktake {
      ...
      deps.timeline('inventory.stocktake_approved', id, { stocktakeNumber: stocktake.stocktakeNumber });
      return this.getStocktakeById(id)!;
    },
    ...
    listStocktakes(limit = 100, offset = 0): Stocktake[] {
      const rows = db
        .prepare(
          `SELECT * FROM stocktakes ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?`,
        )
    ...
    listInventoryAlerts(includeDismissed = false): InventoryAlert[] {
      ...
      return rows.map((row) => ({
        ...
        suggestedReorderQuantity:
          row.suggested_reorder_quantity == null
            ? null
            : Number(row.suggested_reorder_quantity),
    ...
    getInventoryReports(): InventoryReportBundle {
      ...
      const reorderRecommendations = products
        .filter(
          (product) =>
            product.trackStock &&
            (product.stock?.available ?? 0) <= product.minimumStockLevel,
        )
        .map((product) => ({
          productId: product.id,
          sku: product.sku,
          ...
          suggestedReorderQuantity:
            ...
            Math.max(product.minimumStockLevel - (product.stock?.available ?? 0), product.minimumStockLevel, 1),
        }));
