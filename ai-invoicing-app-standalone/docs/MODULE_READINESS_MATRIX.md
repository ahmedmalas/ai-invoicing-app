...
- Current branch: `cursor/slice-39-security-authorization-permissions-19d3`
- Current commit (Slice 39 implementation): `917260bcb94ff21cecb6f2fa626b96d720beacb7`
- Current implemented slice: **Slice 39 — Security, Authorization & Permission Integrity**
...
| Reporting | Partial | Slice 16-17,34-39 | Invoices, Customers, PDF, Audit | TBD | Scope drift into accounting/ledger behavior | Read-only customer statements (JSON/HTML/PDF) are implemented with deterministic filtering and export hardening; Slice 34 adds a shared derived reporting read model for deterministic AR/AP reconciliation, ordering, date filtering, and pagination without materialized reporting tables or ledger persistence; Slice 35 hardens statement read determinism and shared API error contract consistency; Slice 36 verifies reporting parity pre/post restore from recalculated source data; Slice 37 adds concurrency/rollback/idempotency regression coverage proving reporting remains deterministic after concurrent commits and failed transactions; Slice 38 adds regression proof that rejected delete/mutation attempts do not perturb reporting outputs; Slice 39 adds deterministic authentication/authorization protection for reporting/search access. |
