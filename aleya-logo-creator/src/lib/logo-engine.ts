/**
 * Browser-safe port of the deterministic Aleya logo concept generator.
 * Source: ai-invoicing-app-standalone/src/domain/logos/logo-studio.ts
 * Uses Web Crypto / simple hashing instead of node:crypto.
 */
...
/** FNV-1a 32-bit folded into a 16-char hex seed (sync, browser-safe). */
...
function randomId(): string {
  ...
  return `id-${hashSeed(`${Date.now()}-${Math.random()}`)}`;
}

function pick<T>(items: readonly T[], salt: string, index: number): T {
  const start = index % Math.max(salt.length - 1, 1);
  const n = Number.parseInt(salt.slice(start, start + 2), 16);
  return items[Math.abs((Number.isFinite(n) ? n : 0) + index) % items.length]!;
}
...
export function generateLogoConcepts(input: LogoGenerateInput): LogoConcept[] {
  ...
  for (let i = 0; i < count; i += 1) {
    const seed = hashSeed(`${baseSeed}:${i}:${randomId()}`);
    const ideaParts = iconIdeas.split(/[,/|]/);
    concepts.push({
      id: randomId(),
      businessName,
      ...
      iconIdea: ideaParts[i % Math.max(ideaParts.length, 1)]!.trim() || iconIdeas,
...
export function encodeLogoReference(concept: LogoConcept): string {
  ...
  return LOGO_REF_PREFIX + toBase64Url(JSON.stringify(payload));
}
...
export function decodeLogoReference(reference: string | null | undefined): LogoConcept | null {
  ...
  try {
    const json = fromBase64Url(reference.slice(LOGO_REF_PREFIX.length));
    ...
    delete (concept as { svg?: string }).svg;
    return concept;
  } catch {
    return null;
  }
...
export function conceptToEnginePayload(concept: LogoConcept) {
  return {
    ...concept,
    svg: renderLogoSvg(concept),
